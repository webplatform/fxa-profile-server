...
MysqlStore.prototype.getMediawikiProfile = function getMediawikiProfile(email) {
  var defer = P.defer();

  this._connection.query('SELECT username, fullName FROM accounts WHERE normalizedEmail=?', [email], function(err, rows) {
    if (err) {
      return defer.reject(err);
    }
    var result = rows[0];
