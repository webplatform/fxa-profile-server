...
module.exports = {
  handler: function sessionRecover(req, reply) {
    ...
    // Expected format 'Authorization: Sessioin [a-fA-F0-1]{64}', but we dont check the word token
    ...
    if ( sessionToken === false || TOKEN_REGEX.test(sessionToken) === false ) {
      var reason = 'Required Authorization header with Session are either not found is in an invalid format';
    ...
    db.getSessionRecover(sessionToken)
      .done( function (row) {
        if ( ! row['uid'] ) {
            logger.info('sessonRecover, no session found with provided session token', sessionToken);
            reply({
              "code": 410,
              "message": "Gone",
